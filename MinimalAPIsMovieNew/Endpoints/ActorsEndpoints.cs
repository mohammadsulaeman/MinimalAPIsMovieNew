...
namespace MinimalAPIsMovieNew.Endpoints
{
    public static class ActorsEndpoints
    {
        ...
        public static RouteGroupBuilder MapActors(this RouteGroupBuilder group)
        {
            group.MapGet("/", GetAll)
                .CacheOutput(
                c => c.Expire(TimeSpan.FromSeconds(5)).Tag("actors-get"))
            ...
            IMapper mapper, PaginationDTO pagination)
        {
            ...
            var actors = await repository.GetAllActors(pagination);
            ...
            IMapper mapper)
        {
            ...
            if (createActorDTO.Picture is not null)
            {
                var url = await fileStorage.Edit(actorForUpdate.Picture, container,
